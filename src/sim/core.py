...
#     sys.exit()
try:
    libsims = C.CDLL('./lib/libsims.so', mode=C.RTLD_GLOBAL)
...
#     sys.exit()
try:
    libcudmat = C.CDLL('./lib/libcudmat.so', mode=C.RTLD_GLOBAL)
except Exception as e:
...
class Model:
    ...
    ptr_indices = ['t', 'tau', 'X', 'Xi', 'A', 'L', 'B', 'Y', 'P', 'Pi']
    ...
    def __init__(self, n_nodes: int):
        ...
        self.A = np.ctypeslib.as_array(self.model_ptrs[4], shape=(n_nodes,))
        self.L = np.ctypeslib.as_array(self.model_ptrs[5], shape=(n_nodes,))
        self.B = np.ctypeslib.as_array(self.model_ptrs[6], shape=(n_nodes,))
        self.Y = np.ctypeslib.as_array(self.model_ptrs[7], shape=(n_nodes,))
        self.P = np.ctypeslib.as_array(self.model_ptrs[8], shape=(n_nodes,))
        self.Pi = np.ctypeslib.as_array(self.model_ptrs[9], shape=(n_nodes,))
        # internal
        self.epoch = 0
    ...
    def update_t(self, coeff_chi: float, debug_level: int):
        update_t_raw(
            ...
            self.get_raw_ptr('t'),
            self.get_raw_ptr('Xi'),
            self.get_raw_ptr('Xi_prev'), # to add!
